...
if [[ ! -e "${INNOVUS_VIEW_CHECKPOINT}" ]]; then
    echo "[ws-flow][ERROR] Innovus checkpoint not found: ${INNOVUS_VIEW_CHECKPOINT}" >&2
    echo "[ws-flow][INFO] run backend first: ./asic_commercial/ws/scripts/run_innovus.sh all" >&2
    exit 1
fi
