...
if [[ ! -e "${INNOVUS_VIEW_CHECKPOINT}" ]]; then
    echo "[dip-flow][ERROR] Innovus checkpoint not found: ${INNOVUS_VIEW_CHECKPOINT}" >&2
    echo "[dip-flow][INFO] run backend first: ./asic_commercial/dip/scripts/run_innovus.sh all" >&2
    exit 1
fi
...
innovus_args+=(
    -files "${SCRIPT_DIR}/view_innovus_layout.tcl"
