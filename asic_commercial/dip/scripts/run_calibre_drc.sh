...
"${SCRIPT_DIR}/check_backend_inputs.sh" calibre_drc
...
log_file="${LOG_DIR}/calibre_drc_${DESIGN_NAME}.log"
run_dir="${CALIBRE_RUN_DIR}/drc"
mkdir -p "${run_dir}"

echo "[info] launching Calibre DRC with ${tool_bin}"
echo "[info] runset: ${CALIBRE_DRC_RUNSET}"
...
(
    ...
    "${tool_bin}" -drc -hier -runset "${CALIBRE_DRC_RUNSET}" > "${log_file}" 2>&1
)
