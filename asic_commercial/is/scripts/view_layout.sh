...
if [[ ! -e "${INNOVUS_VIEW_CHECKPOINT}" ]]; then
    echo "[is-flow][ERROR] Innovus checkpoint not found: ${INNOVUS_VIEW_CHECKPOINT}" >&2
    echo "[is-flow][INFO] run backend first: ./asic_commercial/is/scripts/run_innovus.sh all" >&2
    exit 1
fi
