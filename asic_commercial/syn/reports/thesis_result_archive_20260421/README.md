...
截至 `2026-04-23` 的实测状态是：

- 新服务器上 `make thesis-check` 与 `make thesis-synth` 已通过
- `SMIC40` 当前交付可以支撑 `DC` 综合口径复现
- `ICC2` 与 `ICC` 探测都已尝试，但当前工艺包里没有发现可直接供 Synopsys 自动数字后端使用的有效 tech/RC 配套

因此如果后续继续引用这份归档，请默认它代表：

- 论文主表的综合与 PPA 对比结果
- 而不是“版图、后仿、回标时序”已经全部打通的结果
